...
# Params
async def visualizeBlogs(result, screenshot, target_url, instructions, base_url):
    # Run the async process that returns an instance of DeeplearningBlogList and screenshot bytes
    

    if result:
        # Convert each blog to a dict (using model_dump from Pydantic v2)
        blogs_data = [blog.model_dump() for blog in result.blogs]

        for blog in blogs_data:
          if blog['blogURL']:
            blog['blogURL'] = f'<a href="{base_url}{blog["blogURL"]}" target="_blank">{blog["title"]}</a>'


        # Build an HTML table if blog data is available
        if blogs_data:
            # Extract headers from the first blog
            headers = blogs_data[0].keys()
            ...
            table_html = "<p>No blog data available.</p>"

        # Display the blog data table
        display(Markdown("### Scraped Blog Data:"))
